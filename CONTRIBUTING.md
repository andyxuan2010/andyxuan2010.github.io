...
Article language is determined by the filename in the same category folder:

- `article-slug.md` is the canonical English article and the default fallback.
- `article-slug.CN.md` is its Chinese translation.
- `article-slug.FR.md` is its French translation.

Use the exact uppercase suffixes `CN` and `FR`. Translations share the English
article's slug and appear through the language selector, not as separate catalog
entries. If a translation is missing, the English article is displayed.
