---
title: "Tailscale + Ansible Across Four Sites"
summary: "Build and validate a Tailscale subnet-routed Ansible management fabric across four Internet-distributed sites for Linux and Windows learners."
document_id: "HOL-09"
...
# Tailscale + Ansible Across Four Sites

> **Document class:** Hands-on Labs guided implementation lab
> **Applicability:** A four-site subnet-routing lab with Tailscale, Ansible, Linux routers, Linux targets, and Windows management endpoints.
...
The execution flow makes cleanup a deliberate boundary: no temporary enrollment key, route advertisement, firewall exception, or lab account should remain after the evidence is captured.
...
Complete the following modules in order. Each module has a checkpoint or validation command before the next scope is opened.

## Prepare the Tailscale policy
...
Start with a policy equivalent to the example in [the enterprise design](../enterprise-solutions/tailscale-ansible-enterprise-solution.md#illustrative-grants-policy). For a lab, the minimum logical requirements are:
...
### Silent installation for multiple direct nodes

Use the following one-line commands from the approved node-management system for direct Tailscale clients. Run the Linux command with root privileges or Ansible `become`; run the Windows command from an elevated PowerShell or deployment context. These commands install the client only. Keep enrollment separate so each node receives its intended auth key, hostname, tag, and route. A host reached only through a subnet router does not need a direct Tailscale client.
...
Windows (64-bit x86 MSI example; use the MSI matching the node architecture):
...
## Prepare each Linux subnet router
...
### Enroll and advertise the site route
...
## Approve and inspect the routes
...
## Enroll the Site B Ansible controller
...
## Validate raw connectivity before Ansible
...
## Optional no-SNAT site-to-site extension
...
## Optional exact-prefix failover lab
...
- [Enterprise Tailscale + Ansible Multi-Site Management Solution](../enterprise-solutions/tailscale-ansible-enterprise-solution.md)
- [Enterprise Multi-Cloud Ansible Automation Reference Architecture](../enterprise-solutions/enterprise-ansible-automation-reference-architecture.md)

## Related repos

- [tailscale/tailscale](https://github.com/tailscale/tailscale) — Tailscale client and subnet-router implementation used by the lab.
- [ansible/ansible](https://github.com/ansible/ansible) — Ansible automation engine used by the controller.
...
- [Run Tailscale unattended](https://tailscale.com/docs/how-to/run-unattended)
